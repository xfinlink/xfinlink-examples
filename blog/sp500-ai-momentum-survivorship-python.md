...
The test uses January 3, 2023 as the index date. The AI candidate list includes NVDA, AVGO, MSFT, AMZN, META, GOOG, ORCL, AMD, ANET, VRT, SMCI, TSM, and PLTR. Built from SEC EDGAR public filings and market data, the analysis combines historical S&P 500 constituents with split-adjusted price returns.
...
4. Adjust closes for stock splits before calculating cumulative returns
5. Compare equal-weight returns for member AI stocks, non-member AI stocks, and SPY

The split adjustment is essential because NVDA and AVGO both had major stock splits during the sample.
...
prices = xfl.prices(ai + ["SPY"], start=as_of, fields=["close", "split_ratio"])
...
adjusted = split_adjusted_prices(prices).ffill()
...
Price sample: 2023-01-03 to 2026-05-29 (854 trading days)
...
Equal-weight return, point-in-time S&P 500 AI members: +491%
Equal-weight return, non-member AI basket:             +1005%
SPY return over same window:                           +99%

AI ticker returns since the index date:
PLTR   +2350%  non-member
NVDA   +1375%  member
VRT     +754%  non-member
AVGO    +707%  member
AMD     +706%  member
...
NVDA  contribution=+153%
AVGO  contribution=+79%
AMD   contribution=+78%
ANET  contribution=+48%
META  contribution=+45%
```

## What this tells us

The AI boom was not only an ex post small-cap or non-index phenomenon. Nine of the selected AI-related tickers were already in the S&P 500 on January 3, 2023: NVDA, AVGO, MSFT, AMZN, META, GOOG, ORCL, AMD, and ANET. Their equal-weight return was +491%, compared with +99% for SPY.

The non-member basket was even stronger at +1005%, led by PLTR at +2350% and VRT at +754%. That result shows why the AI story felt broader than the index. The most explosive returns were outside the point-in-time S&P 500 set.

Within the eligible S&P 500 member basket, NVDA alone contributed +153 percentage points to equal-weight performance. AVGO and AMD each added roughly +79 percentage points. The boom was therefore both investable inside the benchmark and highly concentrated within a few names.
